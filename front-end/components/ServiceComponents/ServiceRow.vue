...
async function Edit(){
    if(await $serviceStore.update(Service.value)){
        editing.value = false;
        emits('update');
    }
}
async function destroy(){
    if(await $serviceStore.destroy(service.id)){
        editing.value = false;
        emits('update');
    }
}
...
function cancle(){
    Service.value.name = service.name;
    Service.value.description = service.description;
    editing.value = false;
}
</script>
...
<template>
    <tr>
        ...
        <td class="px-12 py-4 text-sm font-medium text-gray-700 whitespace-nowrap">
            <input type="text" v-model="Service.description" class="text-sm font-normal text-emerald-500 w-full outline-none" :class="{'border border-black p-3': editing}" :readonly="!editing" >
        </td>
