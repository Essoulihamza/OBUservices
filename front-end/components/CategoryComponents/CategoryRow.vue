...
async function Edit(){
    if(await $categoryStore.update(Category.value)){
        editing.value = false;
        emits('update');
    }
}
async function destroy(){
    if(await $categoryStore.destroy(category.id)){
        editing.value = false;
        emits('update');
    }
}
...
function cancle(){
    Category.value.name = category.name;
...
<template>
    <tr>
        ...
        <td class="px-12 py-4 text-sm font-medium text-gray-700 whitespace-nowrap">
            <input type="text" v-model="Category.description" class="text-sm font-normal text-emerald-500 w-full outline-none" :class="{'border border-black p-3': editing}" :readonly="!editing" >
        </td>
